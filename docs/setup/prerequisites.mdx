---
title: Prerequisites
description: How to configure your development environment for next-forge-paddle.
---

## Operating System

next-forge-paddle is designed to work on macOS, Linux and Windows. While next-forge-paddle itself is platform-agnostic, the tooling and dependencies we use have different requirements.

We've tested and confirmed that next-forge-paddle works on the following combinations of operating systems, Node.js versions and next-forge-paddle versions:

| Operating system | next-forge-paddle version | Node.js version | Notes |
| ---------------- | ----------------- | --------------- | ----- |
...
next-forge-paddle relies on various SaaS products. You will need to create accounts with the following services then set the API keys in your [environment variables](/env):
...
- [Paddle](https://paddle.com), for [payments and billing](/packages/payments).
