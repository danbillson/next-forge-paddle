...
next-forge-paddle has support for product analytics via PostHog — a single platform to analyze, test, observe, and deploy new features.
...
To automatically capture authentication and payment events, we've combined PostHog's Node.js server-side library with Clerk and Paddle webhooks to wire it up as follows:

<Mermaid chart={`
graph TD
  A[User Action in App] -->|Triggers| B[Clerk Webhook]
  A -->|Triggers| E[Paddle Webhook]
  A -->|Client-Side Call| PostHog
  B -->|Sends Data| C1[webhooks/clerk]
  E -->|Sends Data| C2[webhooks/paddle]
